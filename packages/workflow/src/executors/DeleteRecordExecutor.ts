 ...
 *
 * 从数据表中删除记录。
 * 设计器配置：collection（数据表）、filter（ConditionItem[] 匹配条件）
 */
...
export class DeleteRecordExecutor extends DataOperationExecutor {
  async execute(context: ExecutionContext): Promise<ExecutionResult> {
    ...
    try {
      ...
      const affectedCount = await db.destroy(collection, filter);
...
export default DeleteRecordExecutor;

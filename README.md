...
- **运算设计器**（`/designer/computation/:id`）：运算规则编辑器，支持值表达式/条件表达式，含高级模式表达式编辑和结果预览
...
声明式数据运算引擎，基于 JavaScript 表达式语法。

- **表达式类型**：值表达式、条件表达式（普通模式）；完整 JS 语法（高级模式）
- **系统变量**：`$user`、`$now`、`$env`
- **运算场景**：字段联动、数据统计、流程条件、自定义公式
- **API 路由**：`/api/apps/:appId/computations`
